                            ...
                            @foreach ($items as $key => $item)
                                <tr class="@if ($key > 0 && $key % 2) odd @else even @endif">
                                    <td>
                                        ...
                                        @if ($item->structure->cover == "")

                                            <div class="cover-thumbnail"
                                                 style="background-image: url({{ asset('img/no_image.jpg') }});"></div>
                                        @else
                                            <div class="cover-thumbnail"
                                                 style="background-image: url({{$item->structure->cover}})"></div>
                                        @endif
                                    </td>
                                    <td>{{ $item->id }}</td>
                                    ...
                                    <td align="right">
                                        <div class="btn-group">
                                            <a class="btn btn-default btn-xs"
                                               href="{{ route('qr_code.show',$item->id_code) }}">
                                                <i class="fa fa-eye fa-1x fa-lg" aria-hidden="true"></i>
                                            </a>
                                            <a class="btn btn-default btn-xs"
                                               href="{{ route('qr_code.edit',$item->id_code) }}">
                                                <i class="fas fa-pencil-alt fa-1x fa-lg" aria-hidden="true"></i>
                                            </a>

                                            @role('superadministrator')
                                            {!! Form::open(['method' => 'DELETE','route' => ['qr_code.destroy', $item->id_code],'style'=>'display:inline']) !!}
